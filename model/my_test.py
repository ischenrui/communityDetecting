...
# insititution = '材料学院'
#
# path = 'F:\Temp\gmldata\source/%s%s'%(school,insititution)
# g = ig.Graph.Read_GML(path+'.gml')
# # print(list(g.vs))
# # print(list(g.get_edgelist()))
# # 算法包括  Louvain,LPA,GN,CNM
# jsondata = cd_algorithm.detecting(g,'LPA')
#
# #
...
for i in range(600,len(sndic['sch_ins'])):
# for i in range(0,100):
    ...
    try:
        ...
        f = open('F:\\myproject\\python\\communityDetecting\\web\\static\\relation_data\\%s%s%s.txt'%(school,ins,code),'w',encoding='utf-8')
        f.write(jsondata)
        f.close()
    except:
